...
export default function LoginPage() {
  ...
  const handleForgotPassword = async () => {
    setError(null); // Limpa erros antes de tentar

    if (!email || !email.includes('@')) {
      setError({ message: "Digite um e-mail válido para receber o link.", isNewUser: false });
      return;
    }
    ...
    try {
      ...
      setError({ 
        message: "Link enviado! Verifique seu e-mail (e a pasta de spam).", 
        isNewUser: false,
        isSuccess: true 
      });
    } catch (err: any) {
      console.error(err.code);
      setError({ message: "Erro ao enviar e-mail. Verifique o endereço digitado.", isNewUser: false });
    } finally {
  ...
  // --- FUNÇÃO LOGIN COM E-MAIL (CORRIGIDA) ---
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setError(null);

    // .trim() em ambos evita erros de cópia/cola
    const emailLimpo = email.trim();
    const senhaLimpa = password.trim(); 

    try {
      const usuariosRef = collection(db, "usuarios");
      const q = query(usuariosRef, where("email", "==", emailLimpo));
      const querySnapshot = await getDocs(q);

      if (querySnapshot.empty) {
        setError({ message: "E-mail não encontrado no sistema SIADT.", isNewUser: true });
        setLoading(false);
        return;
      }
      ...
      switch (err.code) {
        ...
        case 'auth/invalid-credential':
          msg = "Senha incorreta. Verifique se digitou a nova senha corretamente.";
          break;
        case 'auth/too-many-requests':
          msg = "Acesso bloqueado temporariamente por muitas tentativas. Tente em 5 minutos.";
          break;
        case 'auth/user-disabled':
          msg = "Este usuário foi desativado pelo administrador.";
          break;
      }

      setError({ message: msg, isNewUser: false });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md bg-[#1e293b]/50 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl relative">
        ...
        {error && (
          <div className={`mb-6 p-4 rounded-xl flex items-start gap-3 animate-in fade-in slide-in-from-top-2 ${
            ...
            {error.isSuccess ? <Send size={20} className="shrink-0" /> : error.isNewUser ? <UserPlus className="text-blue-400 shrink-0" size={20} /> : <AlertCircle className="text-red-500 shrink-0" size={20} />}
            <div className="flex-1">
              <p className="text-sm font-medium leading-tight">{error.message}</p>
              {error.isNewUser && <Link href="/Cadastro" className="text-xs font-bold text-blue-400 underline mt-1 block">Criar conta agora</Link>}
            </div>
            <button onClick={() => setError(null)} className="opacity-50 hover:opacity-100 transition-opacity"><X size={18} /></button>
          </div>
        )}
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            ...
            <input 
              ...
              className="w-full bg-[#0f172a] border border-white/10 rounded-xl px-4 py-3.5 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all text-white placeholder:text-gray-600" 
              placeholder="exemplo@siadt.com" 
            />
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              >
                {resetLoading ? "Processando..." : "Esqueci minha senha"}
              </button>
            </div>
          ...
          <button 
            type="submit" 
            disabled={loading} 
            className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-4 rounded-xl shadow-lg shadow-blue-600/20 transition-all flex items-center justify-center gap-2 active:scale-[0.99] disabled:opacity-50"
          >
        ...
        <div className="relative my-8">
          ...
          <div className="relative flex justify-center text-xs uppercase"><span className="bg-[#1e293b] px-2 text-gray-500 font-bold tracking-tighter">Ou continue com</span></div>
        </div>
        ...
        <button 
          ...
          disabled={loading}
          className="w-full bg-white hover:bg-gray-100 text-gray-900 font-bold py-3.5 rounded-xl transition-all flex items-center justify-center gap-3 active:scale-[0.98] disabled:opacity-50"
        >
